...
const server = http.createServer(async (req, res) => {
  ...
  // Image proxy — fetches album art from MA server-side to avoid CORS
  ...
  let filePath = path.join(DIST, rawPath);
  try {
  ...
  try {
    ...
    if (ext === '.html' || ext === '') headers['Cache-Control'] = 'no-store';
...
server.on('upgrade', (req, socket, head) => {
  ...
  proxy.on('error',  (e) => { console.error('[sendspin-proxy] error:', e.message); socket.destroy(); });
});

server.listen(PORT, () => console.log(`[MA Visualizer] Running at http://localhost:${PORT}`));
